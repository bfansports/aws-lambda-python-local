...
help:
	...
	echo "                       Load the proper from /swagger folder using the VERS provided. Processes the file to inject Orchestrate Credentials."
	echo "                       You can also decide to deploy it straight to AWS API Gateway and also deploy live."
	...
	echo "----------------------------------------------------------"
	echo "Connect to a live API: make connect METHOD=<method> ENDPOINT=</path/{id}> [QUERY=<foo=bar&bar=foo>] [PAYLOAD=<filepath>] [NOAUTH=1]"
	echo "                       METHOD: GET, POST, PUT, PATCH, DELETE, etc"
	echo "                       ENDPOINT refers to the URL path after then hostname/stage/"
	echo "                       QUERY refers to the querystring passed to the URL"
	echo "                       You can provide payload with STDIN. Just type your input JSON data, then hit 'ctrl-d' to exit input and send"
